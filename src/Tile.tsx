...
import { Building, PowerSource } from './Building';
...
export class Tile implements TileInterface {
    ...
    savedElevationData: { oldEl: number };
    ...
    doTileUpdate(): boolean {
        ...
        /* Recalculate power generation */
        let newPowerGeneration = 0;
        const ps = this.building as PowerSource;
        if(ps != null && ps.isPowerSource) {
            /* Determine power source */
            if(ps.name.startsWith("Hydro")) {
                newPowerGeneration = 10;
            } else if(ps.name.startsWith("Solar")) {
                /* Being surrounded by buildings doesn't help */
                newPowerGeneration = 25;
                this.getSurroundingTiles().forEach(tile => {
                    if(tile.building != null) {
                        newPowerGeneration = Math.max(newPowerGeneration - 5, 0);
                    }
                });
            } else if(ps.name.startsWith("Windmill")) {
                newPowerGeneration = 25;
                this.getSurroundingTiles().forEach(tile => {
                    if(tile.building != null) {
                        newPowerGeneration = Math.max(newPowerGeneration - 1, 0);
                    } else if(isGroundType(tile.ground, TileGroundType.Water)) {
                        newPowerGeneration += 2; /* boost */
                    }
                });
            }
        }
        let needsRerender = this.currentPowerGeneration != newPowerGeneration;
        this.currentPowerGeneration = newPowerGeneration;
...
export class TileMap extends Array<Tile> {
    ...
    getPower(): number {
        let kwh = 0;
        this.forEach(tile => {
            const ps = (tile.building as PowerSource);
            if(ps == null || !ps.isPowerSource)
                return;
            kwh += tile.currentPowerGeneration;
        });
        return kwh;
    }
}
